...
void create_circle_pnj(int i)
{
    ...
    sfCircleShape_setFillColor(create_dialog[i].circle, create_dialog[i].
        color_circle);
    sfCircleShape_setOutlineColor(create_dialog[i].circle, create_dialog[i].
        color_border_circle);
    sfCircleShape_setOutlineThickness(create_dialog[i].circle, create_dialog[i].
        outlineThickness);
    create_dialog[i].position_circle = (sfVector2f) {create_dialog[i].
...
{
    create_dialog[i].texture = sfTexture_createFromFile
        (create_dialog[i].path_img, NULL);
    create_dialog[i].sprite = sfSprite_create();
    sfSprite_setScale(create_dialog[i].sprite, create_dialog[i].size_sprite);
    sfSprite_setTexture(create_dialog[i].sprite, create_dialog[i].texture,
        sfTrue);
    create_dialog[i].position_circle = (sfVector2f) {create_dialog[i].
        pos_sprite_pnj.x, create_dialog[i].pos_sprite_pnj.y};
...
void create_dialog_text(void)
{
    for (int i = 0; i < size_create_dialog; ++i) {
        ...
        sfText_setCharacterSize(create_dialog[i].text, create_dialog[i].
            size_text);
        sfText_setPosition(create_dialog[i].text, create_dialog[i].
            pos_text);
        sfText_setFillColor(create_dialog[i].text, create_dialog[i].
            color_text);
        sprite_text(i);
        create_pnj(i);
    }

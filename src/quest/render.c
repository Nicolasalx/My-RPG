...
void check_text_move(void)
{
    if (inventory_content.have_totem == false) {
        ...
        quest_totem = true;
    } else if (inventory_content.have_totem == true &&
        inventory_content.have_amuletter
...
void manage_text_part_2(int i)
{
    if (manage_animation.dont_move_amuletter < 2 && manage_animation.
    move_amuletter == true && img_quest_menu[i].text_choose == text_amulet) {
    ...
    if (manage_animation.dont_move_boss < 2 && manage_animation.
        move_boss == true && img_quest_menu[i].text_choose == text_boss) {
        img_quest_menu[i].pos_text = (sfVector2f) {img_quest_menu[i].pos_text.x
            + 750, img_quest_menu[i].pos_text.y};
        sfText_setPosition(img_quest_menu[i].text, img_quest_menu[i].pos_text);

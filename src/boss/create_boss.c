...
void create_boss(void)
{
    for (int j = 0; j < 1; ++j) {
        ...
        sfRectangleShape_setFillColor(boss[j].collision, sfTransparent);
        sfRectangleShape_setOutlineColor(boss[j].collision, sfRed);
        sfRectangleShape_setOutlineThickness(boss[j].collision, 2);
        sfRectangleShape_setSize(boss[j].collision, (sfVector2f) {20 * boss[j].scale.x, 10 * boss[j].scale.y});
        boss[j].clock_move = sfClock_create();
        boss[j].attack_collision = sfRectangleShape_create();
        sfRectangleShape_setFillColor(boss[j].attack_collision, sfTransparent);
        sfRectangleShape_setOutlineColor(boss[j].attack_collision, sfCyan);
        sfRectangleShape_setOutlineThickness(boss[j].attack_collision, 2);
        sfRectangleShape_setSize(boss[j].attack_collision, (sfVector2f) {15 * boss[j].scale.x, 12 * boss[j].scale.y});
        ...
        sfRectangleShape_setSize(boss[j].health_bar_outline, (sfVector2f) {15 * boss[j].scale.x, 2 * boss[j].scale.y});
